...
loop_se_tab$iter = rep(1:100, nrow(sim_list[[1]]) - 1)
...
## calculating LOOP values and the difference in quantile
for(i in 1:40){
  loop_diff_05[[i]] = sim_list2[[i]][1,c(11,12,13)] - sim_list2[[i]][7,c(11,12,13)]
  
  loop_diff_06[[i]] = sim_list2[[i]][2,c(11,12,13)] - sim_list2[[i]][7,c(11,12,13)]
  
  loop_diff_07[[i]] = sim_list2[[i]][3,c(11,12,13)] - sim_list2[[i]][7,c(11,12,13)]
  
  loop_diff_08[[i]] = sim_list2[[i]][4,c(11,12,13)] - sim_list2[[i]][7,c(11,12,13)]
  
  loop_diff_09[[i]] = sim_list2[[i]][5,c(11,12,13)] - sim_list2[[i]][7,c(11,12,13)]
  
  loop_diff_10[[i]] = sim_list2[[i]][6,c(11,12,13)] - sim_list2[[i]][7,c(11,12,13)]
}

loop_se_tab05 = do.call(rbind, loop_diff_05) %>% set_rownames(., 1:length(sim_list2))
loop_se_tab05$model = "model05"

loop_se_tab06 = do.call(rbind, loop_diff_06) %>% set_rownames(., 1:length(sim_list2))
loop_se_tab06$model = "model06"

loop_se_tab07 = do.call(rbind, loop_diff_07) %>% set_rownames(., 1:length(sim_list2))
loop_se_tab07$model = "model07"

loop_se_tab08 = do.call(rbind, loop_diff_08) %>% set_rownames(., 1:length(sim_list2))
loop_se_tab08$model = "model08"

loop_se_tab09 = do.call(rbind, loop_diff_09) %>% set_rownames(., 1:length(sim_list2))
loop_se_tab09$model = "model09"

loop_se_tab10 = do.call(rbind, loop_diff_10) %>% set_rownames(., 1:length(sim_list2))
...
loop_se_tab$iter = rep(1:length(sim_list2), nrow(sim_list2[[1]]) - 1)
...
for(i in 1:length(sim_list3)){
  loop_diff_11[[i]] = sim_list3[[i]][1,c(11,12,13)] - sim_list3[[i]][5,c(11,12,13)]
  
  loop_diff_12[[i]] = sim_list3[[i]][2,c(11,12,13)] - sim_list3[[i]][5,c(11,12,13)]
  
  loop_diff_13[[i]] = sim_list3[[i]][3,c(11,12,13)] - sim_list3[[i]][5,c(11,12,13)]
  
  loop_diff_14[[i]] = sim_list3[[i]][4,c(11,12,13)] - sim_list3[[i]][5,c(11,12,13)]
}

loop_se_tab11 = do.call(rbind, loop_diff_11) %>% set_rownames(., 1:length(sim_list3))
loop_se_tab11$model = "model11"

loop_se_tab12 = do.call(rbind, loop_diff_12) %>% set_rownames(., 1:length(sim_list3))
loop_se_tab12$model = "model12"

loop_se_tab13 = do.call(rbind, loop_diff_13) %>% set_rownames(., 1:length(sim_list3))
loop_se_tab13$model = "model13"

loop_se_tab14 = do.call(rbind, loop_diff_14) %>% set_rownames(., 1:length(sim_list3))
...
loop_se_tab$iter = rep(1:length(sim_list3), nrow(sim_list3[[1]]) - 1)
...
g1 = ggplot(loop_tab, aes(x = LOOP.X50, y = model, group = iter, colour = model))+
  ...
  theme(legend.position = "none",
        axis.title = element_blank()) +
  annotate("label", x = -700, y = 1, label = "Model 15\npreferred") +
  annotate("label", x = 650, y = 1, label = "Alt model\npreferred") +
  scale_y_discrete(limits = rev) +
  scale_colour_manual(values = pals::tableau20(20)[c(1,2,9,10,3,4,7,8,13,14,5,6,17,18)]) + 
