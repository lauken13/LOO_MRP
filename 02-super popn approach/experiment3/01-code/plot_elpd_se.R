...
## elpd values plot ####
xloc = -570
ggplot(pu, aes(x = elpd_loo, y = model, group = iter, colour = model))+
...
xloc2 = 45
ggplot(pu, aes(x = range_pe, y = model, group = iter, colour = model)) +
  geom_point() +
  scale_colour_manual(values = c(paletteer::paletteer_c("ggthemes::Classic Blue", 2),
                                 paletteer::paletteer_c("ggthemes::Red-Gold", 2),
                                 paletteer::paletteer_c("ggthemes::Classic Green", 1),
...
## plot
xloc3 = -5300
ggplot(ph, aes(x = wtd_elpd_loo, y = model, group = iter, colour = model))+
  geom_point(position = position_dodge(width = .5), alpha=0.7) +
  theme(legend.position = "none",
        axis.title = element_blank()) +
  scale_y_discrete(limits = rev) +
  ...
  annotate("label", x = xloc3, y = 1, label = "None")   + xlim(-6900,-5200) 
   geom_errorbarh(mapping = aes(xmin = low_elpd,
                                xmax = upp_elpd),
                  position = position_dodge(width = .5),
                  height = 0, alpha = .5)
...
xloc2 = 1200
ggplot(ph, aes(x = range_pe, y = model, group = iter, colour = model)) +
  geom_point() +
  scale_colour_manual(values = c(paletteer::paletteer_c("ggthemes::Classic Blue", 2),
                                 paletteer::paletteer_c("ggthemes::Red-Gold", 2),
                                 paletteer::paletteer_c("ggthemes::Classic Green", 1),
  ...
  annotate("label", x = xloc2, y = 2,  label = "X2 only") +
  annotate("label", x = xloc2, y = 1, label = "None") +
  labs(title="Range of interval for elpd values (weighted)") + xlim(c(500,1300))
