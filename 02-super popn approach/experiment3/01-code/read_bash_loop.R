...
# sourcing gen_dat()
source(here::here("02-super popn approach/experiment3/01-code/stan code/00-LOO_gen_data.R"))
...
for(ite in c(1:100)) {
  ...
  # calculating popnest
  model06_popnest = apply(popnest_06, c(1,2), function(x)sum(x*popn_ps$Nj)/sum(popn_ps$Nj)) # applying to dimension 1 and 2 in the array (4 chains of 1000 iterations each)
  model11_popnest = apply(popnest_11, c(1,2), function(x)sum(x*popn_ps$Nj)/sum(popn_ps$Nj))
  model13_popnest = apply(popnest_13, c(1,2), function(x)sum(x*popn_ps$Nj)/sum(popn_ps$Nj))
  model13a_popnest = apply(popnest_13a, c(1,2), function(x)sum(x*popn_ps$Nj)/sum(popn_ps$Nj))
  model15_popnest = apply(popnest_15, c(1,2), function(x)sum(x*popn_ps$Nj)/sum(popn_ps$Nj))
  model15a_popnest = apply(popnest_15a, c(1,2), function(x)sum(x*popn_ps$Nj)/sum(popn_ps$Nj))
  ...
  pt_list[ite] = mean(sim1$popn_data$y)
