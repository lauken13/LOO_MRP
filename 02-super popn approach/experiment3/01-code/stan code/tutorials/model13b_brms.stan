...
> model13b = brm(bin_value ~ (1|X1) + (1|X3) + arma(gr = X4, p=1, cov=T), 
+                data = samp_data,
+                backend = "cmdstanr",
...
parameters {
  ...
  vector<lower=-1,upper=1>[Kma] ma;  // moving-average coefficients
...
transformed parameters {
  ...
  chol_cor = cholesky_cor_arma1(ar[1], ma[1], max_nobs_tg);
