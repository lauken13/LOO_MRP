...
iter = c(1:100)
# paletteer::paletteer_c("ggthemes::Red-Gold", 2) # choosing colours
...
# plotting only two models  -----------------------------------------------
pu1 = pu %>% 
  filter(model == c('X1 + X2 + X3 + X4'))

pu2 = pu %>% 
  filter(model == c('*X1 + X2 + X3 + X4'))

## check if AR model is preferred
sum( (pu2$elpd_loo - pu1$elpd_loo) > 0 )
...
(p1 = ggplot(data=pu, aes(x = elpd_loo, y = model, group = iter, colour = model),)+
  ...
  annotate("label", x = -675, y = 5.5, label = "X2 and X4") +
...
ggsave(here::here("02-super popn approach/experiment3/02-results/stan code/plot_loo_unwtd_fx3.png"), p1, width=6, height=7.5, units="in", device="png")
...
(p1a = ggplot(pu, aes(x = range_pe, y = model, group = iter, colour = model)) +
  ...
  theme(legend.position = "none",
        axis.title = element_blank()) +
  annotate("label", x = 20, y = 5.5, label = "X2 and X4") +
  annotate("label", x = xloc2, y = 3.5, label = "X4 only") +
  annotate("label", x = xloc2, y = 2,  label = "X2 only") +
  annotate("label", x = xloc2, y = 1, label = "None") +
  labs(title="Range of interval for elpd values (unweighted)") + xlim(c(6,55)))

ggsave(here::here("02-super popn approach/experiment3/02-results/stan code/plot_loo_unwtd_range_fx3.png"), p1a, width=6, height=7.5, units="in", device="png")
