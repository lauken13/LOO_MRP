...
for(i in iter){
  ...
  calc_tab_m6 = cbind(popnest_sae_X1_m6[[i]], t1)
  calc_tab_m11 = cbind(popnest_sae_X1_m11[[i]], t1)
  calc_tab_m13 = cbind(popnest_sae_X1_m13[[i]], t1)
  calc_tab_m13a = cbind(popnest_sae_X1_m13a[[i]], t1)
  calc_tab_m15 = cbind(popnest_sae_X1_m15[[i]], t1)
  calc_tab_m15a = cbind(popnest_sae_X1_m15a[[i]], t1)
...
sae_X1_tab_m6 = do.call(rbind, calc_list_m6) %>% 
  mutate(model = "X1 + X3")

sae_X1_tab_m11 = do.call(rbind, calc_list_m11) %>% 
  mutate(model = "X1 + X2 + X3")

sae_X1_tab_m13 = do.call(rbind, calc_list_m13) %>% 
  mutate(model = "X1 + X3 + X4")

sae_X1_tab_m13a = do.call(rbind, calc_list_m13a) %>% 
  mutate(model = "*X1 + X3 + X4")

sae_X1_tab_m15 = do.call(rbind, calc_list_m15) %>% 
  mutate(model = "X1 + X2 + X3 + X4")

sae_X1_tab_m15a = do.call(rbind, calc_list_m15a) %>% 
  mutate(model = "*X1 + X2 + X3 + X4")

model_all_pn_tab = rbind(sae_X1_tab_m6, sae_X1_tab_m11, sae_X1_tab_m13,
...
## plot diff in mean
xloc4 = 0.45
(p3 = ggplot(model_sel_pn_tab, aes(x = abs(mean_pe), y = X1, colour = model)) +
