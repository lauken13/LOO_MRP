...
for(i in iter){
  ...
  calc_tab2_m6 = calc_tab_m6 %>% 
    mutate(mean_bias = (as.numeric(`50%`) - as.numeric(mean_prob)),
           low_bias = (as.numeric(`5%`) - as.numeric(mean_prob)),
           upp_bias = (as.numeric(`95%`) - as.numeric(mean_prob)),
...
## plot diff in mean
xloc4 = 0.45
(g4 = ggplot(model_sel_pn_tab, aes(x = abs(mean_bias), y = X4, colour = model)) +
    ...
    scale_colour_manual(values = c("#1C73B1FF", 
                                   ...
                                   "#09622AFF",
                                   "#879195FF")) + 
    stat_summary(aes(group=model), width=1.1, size=0.7,
                 position = position_dodge(width = .8),
                 fun=mean, geom="line", colour=rep(c("#1C73B1FF",
                                                         "#FB964EFF",
                                                         "#09622AFF",
                                                         "#879195FF"), each = 12), alpha=0.8) + #drawing the mean line
    labs(title="Difference in X4-levels estimate and truth",
         y = 'Levels of X4', x = 'Absolute bias' ))

ggsave(here::here("02-super popn approach/experiment4_SAE/02-results/bias_sae_X4_fx3.png"), g4, width=6, height=7.5, units="in", device="png")
