...
for(i in iter){
  popnest_list[[i]] = popnest_summary_list[[i]] %>% 
    ...
    mutate(bias_popn_X50 = as.numeric(popnestX50 - prob_truth)) %>% # recursive diff for pt_list
    mutate(bias_popn_X5 = as.numeric(popnestX5 - prob_truth)) %>% 
    mutate(bias_popn_X95 = as.numeric(popnestX95 - prob_truth)) %>% 
    mutate(ci_width = as.numeric(popnestX95 - popnestX5)) %>% 
    mutate(coveragePopn = ifelse(prob_truth >= popnestX5 & prob_truth <= popnestX95, 1, 0))
}
