...
for(i in 1:length(sim_trip_list)){
...
loop_se_tab11 = do.call(rbind, loop_diff_11) %>% set_rownames(., 1:100)
loop_se_tab11$model = "model11"

loop_se_tab12 = do.call(rbind, loop_diff_12) %>% set_rownames(., 1:100)
loop_se_tab12$model = "model12"

loop_se_tab13 = do.call(rbind, loop_diff_13) %>% set_rownames(., 1:100)
loop_se_tab13$model = "model13"

loop_se_tab14 = do.call(rbind, loop_diff_14) %>% set_rownames(., 1:100)
...
loop_se_tab$iter = rep(1:length(samp_data_list), nrow(sim_trip_list[[1]]) - 1)
