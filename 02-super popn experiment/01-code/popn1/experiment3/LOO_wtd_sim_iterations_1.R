...
for (i in 1:ITE){
 ...
 sim_list[[i]] = cbind(loo_tab, loo_wtd_tab, loo_rank, loo_wtd_rank,
  ...
  coef_list[[i]] = c(coef(model1), coef(model2), coef(model3), coef(model4),
                     coef(model15))
  names(coef_list[[i]]) = c(paste0("01.",names(coef(model1))[grep("*", names(coef(model1)))]),
                            paste0("02.",names(coef(model2))[grep("*", names(coef(model2)))]),
                            paste0("03.",names(coef(model3))[grep("*", names(coef(model3)))]),
                            paste0("04.",names(coef(model4))[grep("*", names(coef(model4)))]),
                            paste0("15.",names(coef(model15))[grep("*", names(coef(model15)))]))
  
  
  save(samp_data_list, sim_list, prob_truth, coef_list, elpd_popnest_list, file="simulated100temp_1.RData")
}

save(samp_data_list, sim_list, prob_truth, coef_list, elpd_popnest_list, file="simulated100_1.RData")
