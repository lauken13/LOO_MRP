...
samp_data_list2 = samp_data_list[1:17]
sim_out = sapply(samp_data_list2, function(x)mean(x$outcome))
prob_truth[1:17]

plot(1:17,prob_truth[1:17], ylim=c(0.26,.3))
...
svy_rake_list = lapply(samp_data_list[1:17], function(x)svydesign(ids=~1, # cluster id, ~1 for no clusters
...
wtd_elpd_se_tab$iter = rep(1:17, 4)
