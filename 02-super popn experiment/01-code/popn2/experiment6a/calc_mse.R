...
ITE = 2
sum((pred_list[[ITE]][n+1: n+n,1] - pt_list[[ITE]])^2)/n


mse_list = list()
for (ite in c(2:93, 95, 97)){
...
g1 = ggplot(mm2, aes(x = value, y = model, group = model, color = model)) +
  ...
  xlim(0,0.13) + 
  annotate("label", x = 0.12, y = 19.5, label = "X2 and X4") +
  annotate("label", x = 0.12, y = 11.5, label = "X4 only") +
  annotate("label", x = 0.12, y = 6,  label = "X2 only") +
  annotate("label", x = 0.12, y = 2, label = "None")
