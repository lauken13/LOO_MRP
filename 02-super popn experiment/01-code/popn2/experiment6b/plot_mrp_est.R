...
g1 = ggplot(pc, aes(x = mean_pe, y = model, group = iter, colour = model))+
  ...
  annotate("label", x = 0.2, y = 19.5, label = "X2 and X4") +
  annotate("label", x = 0.2, y = 11.5, label = "X4 only") +
  annotate("label", x = 0.2, y = 6,  label = "X2 only") +
  annotate("label", x = 0.2, y = 2, label = "None") 
...
# Plotting calibration (bias) ----------------------------------------------------
popnest_tab$mean_pe = as.numeric(popnest_tab[,2] - rep(pt_list[ite], 23)) # recursive diff for pt_list
popnest_tab$low_pe = as.numeric(popnest_tab[,1]  - rep(pt_list[ite], 23))
popnest_tab$upp_pe = as.numeric(popnest_tab[,3]  - rep(pt_list[ite], 23))

popnest_tab$prob_truth = rep(pt_list[ite], 23)

for(ind in 1:nrow(popnest_tab)){
  popnest_tab$cal = ifelse(popnest_tab[ind, 'prob_truth'] >= 
                 popnest_tab[ind, 'popnestX5'] | popnest_tab[ind, 'prob_truth'] <= 
                 popnest_tab[ind, 'popnestX95'], 1, 0) 
}

View(select(popnest_tab, popnestX5, popnestX95, prob_truth, cal))
