...
popn_data$inclusion <- inv_logit_scaled(round(rnorm(J[1], sd=0.5),2)[popn_data$state] + # apply inv-logit for 'simulated' coefficients
                                            round(rnorm(J[2], sd=0.35),2)[popn_data$sex] +
                                            round(rnorm(J[3], sd=0.4),2)[popn_data$eth] +
                                            round(rnorm(J[4], sd=0.2),2)[popn_data$educ] + 
                                            round(rnorm(J[5], sd=0.3),2)[popn_data$age])
...
popn_data[which(popn_data$inclusion == popn_data$inclusion[2]),]
...
model1 = brm(bin_value ~ (1|state) + factor(sex) + (1|eth) +
             ...
             backend = "cmdstanr",
             family = bernoulli(link = "logit"), weight = samp_data$wts,
...
svytotal(~samp_data$elpd_loo1, svy_rake) # weighted elpd
