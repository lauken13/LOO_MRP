...
( g1 = popn_indv_tab %>%
    ...
    labs(tag = paste0(str_pad("Bias-precision", 27, "right"), str_pad("Bias-only", 21, "right"), str_pad("Precision-only", 20, "right"), "Irrelevant")) +
      # geom_line(se = FALSE, colour = "black", stat = "smooth", alpha = .3, size=1.3) +
    theme(plot.tag.position = c(0.527, 0.16),
          plot.tag = element_text(size=18, face="bold")) 
  )


ggsave(g1, width=16, height=10, filename = here("figures/elpd_indv.png"))
```
...
( g2 = popn_indv_tab %>%
    ...
    theme(plot.tag.position = c(0.52, 0.17),
          plot.tag = element_text(size=18, face="bold")) 
    # geom_line(span = 0.5, se=F, colour="black", alpha=0.3, stat="smooth", size=1.3) 
  )
...
( g4 = ggplot(data=ar_tab, aes(x = elpd_diff, y = intscr_diff))+
    ...
    xlab("LOO (AR prior) - LOO (non-AR prior)")+
...
ggsave(g4, width=12, height=10, filename = here("figures/elpd_AR_500.png"))
```
