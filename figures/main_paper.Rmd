...
res_list_base <- readRDS(here('code/simDesign1/data/res_list_N02_1000.rds'))
res_list_sae <- readRDS(here('code/simDesign2/data/res_list_sae_500_wtd.rds')) # change to *_sae_1000_wtd.rds for sample size n = 1000
...
indv_all_tab_sae = res_list_sae$indv_all_tab
popn_indv_tab_sae = res_list_sae$popn_indv_tab

model_sae_X1_tab = res_list_sae$model_sae_X1_tab
model_sae_X2_tab = res_list_sae$model_sae_X2_tab
model_sae_X3_tab = res_list_sae$model_sae_X3_tab
model_sae_X4_tab = res_list_sae$model_sae_X4_tab
```
...
t3 = popn_indv_tab %>%
  ...
  rename(intervalScr = popnInd_intervalScr_mean) %>% 
  select(intervalScr, model, model_score, popnInd, type)
...
# merging sample and population ind
( g1a = union(t2,t3) %>% 
    mutate(popnInd = factor(popnInd),
           popnInd = fct_recode(popnInd, `Sample` = "0", `Population` = "1")) %>% 
...
## ELPD and population MRP interval score - X2 and X4 experiment
```{r X2 and X4 experiment}
res_list_X2 <- readRDS(here('code/simDesign1/data/res_list_N02_X2.rds'))
res_list_X4 <- readRDS(here('code/simDesign1/data/res_list_N02_X4.rds'))

popn_indv_tab_X2 = res_list_X2$popn_indv_tab %>% 
  mutate(var = "X2")
popn_indv_tab_X4 = res_list_X4$popn_indv_tab %>% 
  mutate(var = "X4")

# joining two sets together to plot 2 x 2
popn_indv_tab_all = bind_rows(popn_indv_tab_X2, popn_indv_tab_X4)

corr_popn_intervalscore_all <- popn_indv_tab_all %>%
  pivot_longer(cols = c("elpd_loo","wtdElpd_loo"), names_to = "type", values_to = "model_score")%>%
  mutate(type = factor(type),
         type = fct_recode(type, `PSIS-LOO` = "elpd_loo", `WTD-PSIS-LOO` = "wtdElpd_loo"))%>%
  group_by(var, type)%>%
  summarise(cor_val = paste0("cor: ",round(cor(model_score, MRP_intervalScr),2)))%>%
  mutate(xloc = c(-390,-4000),yloc = rep(2.5,2))

popn_indv_tab_all$model = fct_relevel(popn_indv_tab_all$model, 'X2 + X4',
                                     'X1 + X2 + X4', 
                                     'X2 + X3 + X4',
                                     'X1 + X2 + X3 + X4',
                                     'X4', 'X1 + X4', 
...
( g2a = popn_indv_tab_all %>%
    ...
    ggplot(., aes(x = model_score, y = MRP_intervalScr))+
    geom_point(size=3, alpha = .5, aes( colour = model, shape = model)) +
    geom_text(data  = corr_popn_intervalscore_all,
              mapping = aes(x = xloc, y = yloc, label = cor_val), size=9) +
    scale_colour_manual(values = colour_palette_var_base, labels = leg_lab) + 
    scale_shape_manual(values = shape_base, labels = leg_lab) +
...
ggsave(g2a, width=16, height=10, filename = here("figures/elpd_popn_X2X4.png"))
```
...
xloc1 = -0.08; xloc2 = 0.17; yloc1 = -1.5; yloc2 = 1.8
...
```{r}
## sample size n = 1000 ####
# rerun code from line 575-601 before running these
ar_tab = sae_clean %>%
  filter(full_model, sae == "X4") %>%
...
freq_sae_base = sae_clean_base %>% # run code 364-390 before
  group_by(sae, group) %>%
