...
nhmodel_allvar_dr2 = brm(high_bp ~  (1|age) + (1|ethnicity)  + phys_act + overweight + (1|BMI_range) + 
                           (1|diabetes)  + (1|marital_status) + trb_sleep + (1|educ) + (1|alc_exc) +  
                           (1|pov_level) +  (1|sodium_lvl) + (1|elst_status) + (1|hiv_test) + (1|urn_vol) +
                           gender  +  smk_tobcig,
                         data = nhsub_dr2,
                         seed = 2048,
                         chain = 2,  
...
nhmodel_biasprec_dr2 = brm(high_bp ~  (1|age) + (1|ethnicity)  + phys_act + overweight + (1|BMI_range) + 
                             (1|diabetes)  + (1|marital_status) + trb_sleep,
                           data = nhsub_dr2,
                           seed = 2048,
                           chain = 2,  
                           backend = "cmdstanr", silent = 2,
...
# fitting bias only 
nhmodel_bias_dr2 = brm(high_bp ~  (1|age) + (1|ethnicity)  + phys_act + overweight + (1|BMI_range) + 
                         (1|diabetes),
                       data = nhsub_dr2,
                       seed = 2048,
                       chain = 2,  
...
# inconsequential
nhmodel_inc_dr2 = brm(high_bp ~ (1|educ) + (1|alc_exc) + (1|pov_level) +  (1|sodium_lvl) + (1|elst_status) + 
                        (1|hiv_test) + (1|urn_vol),
                      data = nhsub_dr2,
                      seed = 2048,
                      chain = 2,  
                      backend = "cmdstanr", silent = 2,
...
nhmodel_allvar_fas = brm(high_bp ~  (1|ethnicity) + overweight + (1|age) + (1|marital_status)  + phys_act  +
                           (1|BMI_range) + (1|diabetes) + trb_sleep + (1|elst_status) +  (1|urn_vol) +
                           gender +  (1|educ) +  (1|alc_exc) + (1|pov_level) + smk_tobcig +
                           (1|sodium_lvl) + (1|hiv_test) ,
                         data = nhsub_fas,
                         seed = 2048,
                         chain = 2,  
                         backend = "cmdstanr", silent = 2,
...
# fitting precision only 
nhmodel_prec_fas = brm(high_bp ~  (1|age) + (1|marital_status)  + phys_act  +
                         (1|BMI_range) + (1|diabetes) + trb_sleep,
                       data = nhsub_fas,
                       seed = 2048,
                       chain = 2,  
...
# fitting ignorable
nhmodel_ign_fas = brm(high_bp ~ gender +  (1|educ) +  (1|alc_exc) + (1|pov_level) + smk_tobcig +
                        (1|sodium_lvl) + (1|hiv_test),
                      data = nhsub_fas,
                      seed = 2048,
                      chain = 2,  
